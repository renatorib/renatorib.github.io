...
const lexer = (input /*: string */) => {
  ...
  while (pointer < input.length) {
    ...
    let char = input[pointer]; // takes the char corresponding to the pointer position

    /**
     * performs logic based on the char found
     */

    pointer++; // advances the pointer to continue the execution of the while
  }
};
```
